	...
	public void onEnable() {
		...
		ClanVentBus.subscribe(PlayerKillPlayerEvent.class, Vent.Priority.HIGH, (e, subscription) -> {
			...
			if (ClansAPI.getInstance().isInClan(p.getUniqueId())) {
				...
				if (b != null) {
					if (ClansAPI.getData().isTrue("Addon.Bounty.settings.take-from-killed")) {
						// take from wallet
						if (ClansAPI.getInstance().isInClan(e.getVictim().getUniqueId())) {
							...
							if (ClansAPI.getData().isTrue("Addon.Bounty.settings.announce-defeat")) {
								String format = ClansAPI.getData().getMain().getRoot().getString("Addon.Bounty.settings.defeat-message")
							...
							if (t.has(b.getAmount())) {
								...
								t.setBalance(newBal);
								if (ClansAPI.getData().getMain().getRoot().getString("Addon.Bounty.settings.deposit-type").equals("PLAYER")) {
								...
								} else if (ClansAPI.getData().getMain().getRoot().getString("Addon.Bounty.settings.deposit-type").equals("CLAN")) {
						...
						if (id != null) {
							...
							if (BountyList.get(c, id) == null) {
								...
								if (has) {
									...
									String format = ClansAPI.getData().getMain().getRoot().getString("Addon.Bounty.settings.called-message")
									...
									clanFile.getRoot().set("bounties." + Clan.ACTION.getUserID(e.getArgs()[1]).toString(), amount);

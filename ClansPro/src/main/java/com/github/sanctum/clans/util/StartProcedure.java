...
public class StartProcedure {
	...
	public void registerDefaults() {
		...
		new Registry<>(Listener.class).source(ClansAPI.getInstance().getPlugin()).pick("com.github.sanctum.clans.util.listener").operate(listener -> {
		...
		for (EventCycle e : CycleList.getRegisteredCycles()) {
			if (e.persist()) {
				try {
					...
					instance.getLogger().severe("- An issue occurred while enabling addon " + e.getName());
					Schedule.sync(() -> CycleList.getRegisteredCycles().removeIf(c -> c.getName().equals(e.getName()))).wait(1);
					ex.printStackTrace();
				}
			} else {
				sendBorder();

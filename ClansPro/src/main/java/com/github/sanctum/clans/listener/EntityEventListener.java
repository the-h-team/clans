		...
		if (e.getEntity() instanceof Player) {
			...
			if (ClansAPI.getInstance().isInClan(p.getUniqueId())) {
				if (ClansAPI.getData().prefixedTagsAllowed()) {
					Schedule.sync(() -> ClanDisplayName.set(p, ClansAPI.getData().prefixedTag(ClansAPI.getInstance().getClan(p.getUniqueId()).getColor(), ClansAPI.getInstance().getClan(p.getUniqueId()).getName()))).run();
				}
			}
		}
	}

...
public class EntityEventListener implements Listener {
	...
	public EntityEventListener() {
		...
		Vent.subscribe(new Vent.Subscription<>(DefaultEvent.PlayerDamagePlayer.class, ClansPro.getInstance(), Vent.Priority.HIGH, (event, subscription) -> {
			...
			if (event.isPhysical()) {
				...
				if (!ClansPro.getInstance().dataManager.getMain().getConfig().getStringList("Clans.world-whitelist").contains(event.getPlayer().getWorld().getName())) {
					return;
				}

			...
			for (Resident res : ClansAPI.getData().RESIDENTS) {
				if (res.getPlayer().getName().equals(p.getName())) {
					...
					if (!ClansAPI.getData().INHABITANTS.contains(p)) {
						if (titlesAllowed) {
							try {
								titleContext.put("W-TITLE", MessageFormat.format(ClansAPI.getData().getMain().getRoot().getString("Clans.land-claiming.wilderness.title"), res.getLastKnown().getClan().getName()));
								titleContext.put("W-SUB-TITLE", MessageFormat.format(ClansAPI.getData().getMain().getRoot().getString("Clans.land-claiming.wilderness.sub-title"), res.getLastKnown().getClan().getName()));
								p.sendTitle(getClaimUtil().color(titleContext.get("W-TITLE")), getClaimUtil().color(titleContext.get("W-SUB-TITLE")), 10, 25, 10);
							} catch (NullPointerException e) {
								titleContext.put("W-TITLE", MessageFormat.format(ClansAPI.getData().getMain().getRoot().getString("Clans.land-claiming.wilderness.title"), "Un-claimed"));
								titleContext.put("W-SUB-TITLE", MessageFormat.format(ClansAPI.getData().getMain().getRoot().getString("Clans.land-claiming.wilderness.sub-title"), "Un-claimed"));
								p.sendTitle(getClaimUtil().color(titleContext.get("W-TITLE")), getClaimUtil().color(titleContext.get("W-SUB-TITLE")), 10, 25, 10);
						...
						if (ClansAPI.getData().isTrue("Clans.land-claiming.send-messages")) {
							getClaimUtil().sendMessage(p, MessageFormat.format(ClansAPI.getData().getMain().getRoot().getString("Clans.land-claiming.wilderness.message"), res.getLastKnown().getClan().getName()));
						}

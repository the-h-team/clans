...
public class UI {
	...
	private static final Map<UUID, ClanEdit> CLAN_EDIT_OPERATION_MAP = new HashMap<>();
	private static final Map<UUID, MemberEdit.Option> MEMBER_OPTION_MAP = new HashMap<>();
	private static final Map<UUID, MemberEdit> MEMBER_EDIT_OPERATION_MAP = new HashMap<>();
	...
	public static ClanEdit.Option getClanEditOption(UUID id) {
		return CLAN_OPTION_MAP.getOrDefault(id, null);
	}

	public static ClanEdit getClanEditOperation(UUID id) {
		return CLAN_EDIT_OPERATION_MAP.getOrDefault(id, null);
	}

	public static MemberEdit.Option getMemberEditOption(UUID id) {
		return MEMBER_OPTION_MAP.getOrDefault(id, null);
	}

	public static MemberEdit getMemberEditOperation(UUID id) {
					...
					builder.setClick((player, text, args) -> {
						if (args.length == 0) {
							...
							if (clanId != null) {
								...
								Clan.ACTION.sendMessage(player, "&3This clan name is available.");
							}
							return;
						}
						for (String arg : args) {
							...
							if (clanId != null) {
								...
								Clan.ACTION.sendMessage(player, "&3Clan name '" + arg + "' is available.");
							}
						}
					});
				}).get();
	...
	public static Menu select(Singular type, HUID id) {
		...
		if (type == Singular.CLAN_EDIT) {
			builder = new MenuBuilder(InventoryRows.THREE, StringUtils.use("&0&l» " + c.getColor() + c.getName() + " settings").translate())
					...
					.addElement(c.getMembers().stream().filter(m -> m.getPriority() == RankPriority.HIGHEST).findFirst().get().getHead())
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.PASSWORD);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter a new password with &f/c respond <context...>");
						p.closeInventory();
					})
					.assignToSlots(4)
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.CLAIMS_GIVE);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter an amount with &f/c respond <context...>");
						p.closeInventory();
					})
					.assignToSlots(1)
					.addElement(new ItemStack(Material.SLIME_BALL))
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.POWER_GIVE);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter an amount with &f/c respond <context...>");
						p.closeInventory();
					})
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.MONEY_GIVE);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter an amount with &f/c respond <context...>");
						p.closeInventory();
					})
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.CLAIMS_TAKE);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter an amount with &f/c respond <context...>");
						p.closeInventory();
					})
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.POWER_TAKE);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter an amount with &f/c respond <context...>");
						p.closeInventory();
					})
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.MONEY_TAKE);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter an amount with &f/c respond <context...>");
						p.closeInventory();
					})
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.TAG);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter a new name with &f/c respond <context...>");
						p.closeInventory();
					})
					.assignToSlots(22)
					.addElement(new ItemStack(Material.WRITTEN_BOOK))
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.DESCRIPTION);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter a new description with &f/c respond <context...>");
						p.closeInventory();
					})
					.assignToSlots(12)
					.addElement(new ItemStack(Material.WRITTEN_BOOK))
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.COLOR);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Enter a new color with &f/c respond <context...>");
						p.closeInventory();
					})
					.assignToSlots(14)
					.addElement(getBack())
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.UPDATE_BASE);
						new ClanEdit(c, p).execute();
					...
					.setAction(click -> {
						Player p = click.getPlayer();
						CLAN_OPTION_MAP.put(p.getUniqueId(), ClanEdit.Option.CLOSE);
						CLAN_EDIT_OPERATION_MAP.put(p.getUniqueId(), new ClanEdit(c, p));
						Clan.ACTION.sendMessage(p, "&3Confirm deletion with &f/c respond confirm");
						Clan.ACTION.sendMessage(p, "&cCancel with &f/c respond cancel");
						p.closeInventory();
					})
					.assignToSlots(26);
	...
	public static Menu select(Singular type, UUID... uuids) {
		...
		switch (type) {
			...
			case MEMBER_INFO:
				...
				builder = new MenuBuilder(InventoryRows.THREE, Clan.ACTION.color(test))
						.cancelLowerInventoryClicks(false)
						...
						.assignToSlots(13)
						.addElement(CustomHead.Manager.get(associate.getPlayer()) != null ? new Item.Edit(associate.getHead()).setFlags(ItemFlag.HIDE_ENCHANTS).addEnchantment(Enchantment.ARROW_DAMAGE, 69).build() : new ItemStack(ClansAPI.getData().getItem("player")))
						.setLore(StringUtils.use(bio + " &f-" + associate.getNickname()).translate(), StringUtils.use("&e&oClick to teleport to me.").translate())
						...
						.setAction(click -> {
							if (associate.getPlayer().isOnline()) {
								...
								Message.form(click.getPlayer()).setPrefix(ClansAPI.getInstance().getPrefix().joined()).send("&cIm not online at the moment, hit me up later!");
							}
						})
						.assignToSlots(4)
						.addElement(new ItemStack(Material.BOOK))
						...
						.setAction(click -> {
							...
							if (p.hasPermission("clanspro.admin")) {
								...
								if (LabyrinthProvider.getInstance().isLegacy()) {
									click.getInventoryView().getTopInventory().setItem(26, new ItemStack(Items.getMaterial("STAINED_GLASS_PANE")));
								} else {
									click.getInventoryView().getTopInventory().setItem(26, new ItemStack(Items.getMaterial("GRAYSTAINEDGLASSPANE")));
								}
							}
			...
			case MEMBER_EDIT:
				Clan cl = Optional.ofNullable(uuid)
						.map(ClansAPI.getInstance()::getClanID)
						.map(HUID::toString)
				...
				builder = new MenuBuilder(InventoryRows.THREE, StringUtils.use("&0&l» " + cl.getColor() + player.getName() + " settings").translate())
						...
						.setAction(click -> {
							Player p = click.getPlayer();
							MEMBER_OPTION_MAP.put(p.getUniqueId(), MemberEdit.Option.BIO);
							MEMBER_EDIT_OPERATION_MAP.put(p.getUniqueId(), new MemberEdit(cl, p, player.getUniqueId()));
							Clan.ACTION.sendMessage(p, "&3Enter a new bio with &f/c respond <context...>");
							p.closeInventory();
						})
						.assignToSlots(4)
						.addElement(new ItemStack(Material.ENCHANTED_BOOK))
						...
						.setAction(click -> {
							Player p = click.getPlayer();
							MEMBER_OPTION_MAP.put(p.getUniqueId(), MemberEdit.Option.NICKNAME);
							MEMBER_EDIT_OPERATION_MAP.put(p.getUniqueId(), new MemberEdit(cl, p, player.getUniqueId()));
							Clan.ACTION.sendMessage(p, "&3Enter a new name with &f/c respond <context...>");
							p.closeInventory();
						})
						...
						.setAction(click -> {
							Player p = click.getPlayer();
							MEMBER_OPTION_MAP.put(p.getUniqueId(), MemberEdit.Option.SWITCH_CLANS);
							MEMBER_EDIT_OPERATION_MAP.put(p.getUniqueId(), new MemberEdit(cl, p, player.getUniqueId()));
							Clan.ACTION.sendMessage(p, "&3Enter a clan name with &f/c respond <context...>");
							p.closeInventory();
						})
						.assignToSlots(12)
						...
						.setAction(click -> {
							Player p = click.getPlayer();
							MEMBER_OPTION_MAP.put(p.getUniqueId(), MemberEdit.Option.KICK);
							new MemberEdit(cl, p, player.getUniqueId()).execute();
							clearOperations(p.getUniqueId());
							p.closeInventory();
						})
						.assignToSlots(11)
						...
						.setAction(click -> {
							Player p = click.getPlayer();
							MEMBER_OPTION_MAP.put(p.getUniqueId(), MemberEdit.Option.PROMOTE);
							new MemberEdit(cl, p, player.getUniqueId()).execute();
							clearOperations(p.getUniqueId());
							p.closeInventory();
						})
						...
						.setAction(click -> {
							Player p = click.getPlayer();
							MEMBER_OPTION_MAP.put(p.getUniqueId(), MemberEdit.Option.DEMOTE);
							new MemberEdit(cl, p, player.getUniqueId()).execute();
							clearOperations(p.getUniqueId());
							p.closeInventory();
						})
			...
			case ARENA_SETUP:
				...
				break;
			case LANG_WINDOW:
				builder = new MenuBuilder(InventoryRows.ONE, StringUtils.use(" &0&l» &ePick a language").translate())
						.cancelLowerInventoryClicks(false)
						.addElement(new Item.Edit(Material.BOOK).setTitle("&aEnglish").build())
						.setAction(click -> {
							click.getPlayer().closeInventory();
							...
							nm.reload();
							Message.form(click.getPlayer()).setPrefix(ClansAPI.getInstance().getPrefix().joined()).send("&a&oTranslated back to default english.");
						...
						.assignToSlots(0)
						.addElement(new Item.Edit(Material.BOOK).setTitle("&bPortuguese").build())
						.setAction(click -> {
							click.getPlayer().closeInventory();
							FileManager config = ClansAPI.getInstance().getFileList().find("Config", "Configuration");
							config.delete();
							...
							nm.reload();
							Message.form(click.getPlayer()).setPrefix(ClansAPI.getInstance().getPrefix().joined()).send("&a&oAgora traduzido para o brasil!");
				...
				break;
			case RELOAD_WINDOW:
				builder = new MenuBuilder(InventoryRows.ONE, StringUtils.use(" &0&l» &eReload Files").translate())
						.cancelLowerInventoryClicks(false)
						.addElement(new Item.Edit(Material.POTION).setTitle("&aConfig.yml").build())
						.setAction(click -> {
							UI.select(Singular.SETTINGS_WINDOW).open(click.getPlayer());
							ClansAPI.getData().getMain().reload();
							Message.form(click.getPlayer()).setPrefix(ClansAPI.getInstance().getPrefix().joined()).send("&aConfig file 'Config' reloaded.");
						})
						.assignToSlots(0)
						.addElement(new Item.Edit(Material.POTION).setTitle("&5Messages.yml").build())
						.setAction(click -> {
							UI.select(Singular.SETTINGS_WINDOW).open(click.getPlayer());
							DataManager.FileType.MISC_FILE.get("Messages", "Configuration").reload();
							Message.form(click.getPlayer()).setPrefix(ClansAPI.getInstance().getPrefix().joined()).send("&aConfig file 'Messages' reloaded.");
						})
						.assignToSlots(2)
						.addElement(new Item.Edit(Material.POTION).setTitle("&2&lAll").build())
						.setAction(click -> {
							Player p = click.getPlayer();
							FileManager config = ClansAPI.getInstance().getFileList().find("Config", "Configuration");
						...
						.assignToSlots(4)
						.addElement(new Item.Edit(Material.POTION).setTitle("&eLang Change").build())
						.setAction(click -> {
							Player p = click.getPlayer();
							UI.select(Singular.LANG_WINDOW).open(p);
						})
						.assignToSlots(6)
						.addElement(getBack())
						.setAction(click -> {
	...
	public static Menu.Paginated<Clan> browse(Paginated type) {
		switch (type) {
			case CLAN_ROSTER:
				return new PaginatedBuilder<>(ClansAPI.getInstance().getClanManager().getClans().list())
						.forPlugin(ClansAPI.getInstance().getPlugin())
						...
						.build();
			case CLAN_TOP:
				return new PaginatedBuilder<>(Clan.ACTION.getMostPowerful())
						.forPlugin(ClansAPI.getInstance().getPlugin())
	...
	public static Menu.Paginated<ClanAssociate> view(Clan c) {
		...
		return new PaginatedBuilder<>(new ArrayList<>(c.getMembers()))
				.forPlugin(ClansAPI.getInstance().getPlugin())
	...
	public static Menu.Paginated<Clan> browseEdit() {
		return new PaginatedBuilder<>(Clan.ACTION.getMostPowerful())
				.forPlugin(ClansAPI.getInstance().getPlugin())
				.setTitle(Clan.ACTION.color("&0&l» &3&lSelect a clan"))
				.setAlreadyFirst(Clan.ACTION.color(Clan.ACTION.alreadyFirstPage()))
				.setAlreadyLast(Clan.ACTION.color(Clan.ACTION.alreadyLastPage()))
				.setNavigationLeft(getLeft(), getLeftSlot(), PaginatedClickAction::sync)
				.setNavigationRight(getRight(), getRightSlot(), PaginatedClickAction::sync)
	...
	public static Menu.Paginated<ClanAssociate> edit(Clan c) {
		return new PaginatedBuilder<>(new ArrayList<>(c.getMembers()))
				.forPlugin(ClansAPI.getInstance().getPlugin())
				.setTitle(Clan.ACTION.color("&0&l» &3&lSelect a member"))
				.setAlreadyFirst(Clan.ACTION.color(Clan.ACTION.alreadyFirstPage()))
				.setAlreadyLast(Clan.ACTION.color(Clan.ACTION.alreadyLastPage()))
				.setNavigationLeft(getLeft(), getLeftSlot(), PaginatedClickAction::sync)
				.setNavigationRight(getRight(), getRightSlot(), PaginatedClickAction::sync)
	...
	public enum Singular {
		ROSTER_ORGANIZATION, MEMBER_INFO, CYCLE_ORGANIZATION, SETTINGS_WINDOW, RELOAD_WINDOW, LANG_WINDOW, SHIELD_TAMPER, CLAN_EDIT, MEMBER_EDIT, ARENA_SETUP
	}

	public static class ClanEdit {
		...
		protected ClanEdit(Clan target, Player executor) {
			this.target = target;
			this.executor = executor;
			this.option = getClanEditOption(executor.getUniqueId());
		}
		...
		public void execute() {
			switch (option) {
				case PASSWORD:
					...
					Clan.ACTION.sendMessage(executor, "&aClan " + getTarget().getName() + " password changed to &f: " + context);
					break;
				case TAG:
					Clan.ACTION.sendMessage(executor, "&aClan " + getTarget().getName() + " name changed to &f: " + context);
					getTarget().setName(context);
					break;
				case COLOR:
					getTarget().setColor(context);
					Clan.ACTION.sendMessage(executor, "&aClan " + getTarget().getName() + " color changed to &r: " + context + "Example");
					break;
				case DESCRIPTION:
					getTarget().setDescription(context);
					Clan.ACTION.sendMessage(executor, "&aClan " + getTarget().getName() + " description changed to &f: " + context);
					break;
				case CLAIMS_GIVE:
				...
				case UPDATE_BASE:
					...
					Clan.ACTION.sendMessage(executor, "&aClan " + getTarget().getName() + " base location updated to current location");
					break;
				case CLOSE:
	...
	public static class MemberEdit {
		...
		protected MemberEdit(Clan target, Player executor, UUID member) {
			this.target = target;
			this.member = member;
			this.executor = executor;
			this.option = getMemberEditOption(executor.getUniqueId());
		}
			...
			switch (option) {
				case BIO:
					if (associate != null) {
						...
						Clan.ACTION.sendMessage(executor, "&a&oPlayer " + Bukkit.getOfflinePlayer(member).getName() + " bio changed to:&r " + context);
					}
					break;
				case NICKNAME:
					if (associate != null) {
						associate.changeNickname(context);
						Clan.ACTION.sendMessage(executor, "&a&oPlayer " + Bukkit.getOfflinePlayer(member).getName() + " nickname changed to:&r " + context);
					}
					break;
				case PROMOTE:
					RankPriority before1 = associate.getPriority();
					if (before1.toInt() == 2) {
						Clan.ACTION.sendMessage(executor, "&a&oPlayer " + Bukkit.getOfflinePlayer(member).getName() + " is already the highest rank:&r " + associate.getRankTag());
						break;
					}
					Clan.ACTION.promotePlayer(member);
					Clan.ACTION.sendMessage(executor, "&a&oPlayer " + Bukkit.getOfflinePlayer(member).getName() + " promoted to:&r " + associate.getRankTag());
					break;
				case KICK:
					Bukkit.dispatchCommand(executor, "cla kick " + Bukkit.getOfflinePlayer(member).getName());
					break;
				case DEMOTE:
					RankPriority before = associate.getPriority();
					if (before.toInt() == 0) {
						Clan.ACTION.sendMessage(executor, "&a&oPlayer " + Bukkit.getOfflinePlayer(member).getName() + " is already the lowest rank:&r " + associate.getRankTag());
						break;
					}
					Clan.ACTION.demotePlayer(member);
					Clan.ACTION.sendMessage(executor, "&a&oPlayer " + Bukkit.getOfflinePlayer(member).getName() + " demoted to:&r " + associate.getRankTag());
					break;
				case SWITCH_CLANS:
					Bukkit.dispatchCommand(executor, "cla kick " + Bukkit.getOfflinePlayer(member).getName());

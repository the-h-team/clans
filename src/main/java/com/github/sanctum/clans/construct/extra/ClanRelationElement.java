		...
		switch (relation) {
			case Ally:
				if (clan instanceof DefaultClan) {
					return ((DefaultClan)clan).getAllyList().stream().map(HUID::fromString).map(huid -> ClansAPI.getInstance().getClanManager().getClan(huid)).collect(Collectors.toList());
				}
				return clan.getRelation().getAlliance().get(Clan.class);
			case Enemy:
				if (clan instanceof DefaultClan) {
					return ((DefaultClan)clan).getEnemyList().stream().map(HUID::fromString).map(huid -> ClansAPI.getInstance().getClanManager().getClan(huid)).collect(Collectors.toList());
				}

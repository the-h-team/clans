package com.github.sanctum.clans.model.addon.vaults;
...
public class VaultMenu extends Menu {
	...
	public VaultMenu(String clanName) {
		...
		if (this.c != null) {
			...
			this.key = clanName;
			this.close = close -> {
				FileManager manager = Clan.Addon.getAddon(VaultsAddon.class).getFile(Configurable.Type.JSON, "data");
	...
	ItemStack[] getInventoryContents(HUID id) {
		...
		Clan.Addon vaults = Clan.Addon.getAddon(VaultsAddon.class);
		if (vaults != null) {

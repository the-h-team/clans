package com.github.sanctum.clans.model.addon.map;

import com.github.sanctum.clans.model.ClanAddonRegistry;
import com.github.sanctum.clans.model.ClanVentBus;
import com.github.sanctum.clans.model.addon.map.event.AsyncMapDrawEvent;
import com.github.sanctum.clans.model.addon.map.event.AsyncMapFormatEvent;
import com.github.sanctum.clans.model.addon.map.event.ChangeChunkInWorldEvent;
import com.github.sanctum.clans.model.addon.map.structure.ChunkPosition;
import com.github.sanctum.clans.model.addon.map.structure.MapPoint;
...
public class MapController implements Listener {
	...
	public void onChangeChunkInWorld(ChangeChunkInWorldEvent e) {
		if (players.contains(e.getPlayer())) {
			...
			if (optional.isPresent()) sendMapLoc(e.getPlayer(), e.getToChunk());
		}
	}
	...
	public void onFormat(AsyncMapFormatEvent e) {
		...
		Clan.Addon ec = ClanAddonRegistry.getInstance().get("Map");

		if (ec != null) {

			boolean enhanced = Clan.Addon.getServicesManager().getRegistration(Boolean.class, ec).getService();
			...
			if (enhanced) {
				...
				if (LabyrinthProvider.getInstance().isLegacy()) {
					...
					Clan.Addon.getServicesManager().unregisterAll(ec);
					Clan.Addon.getServicesManager().register(false, ec, ServicePriority.Highest);
					return;
				}
	...
	public static void sendMapCurrentLoc(final Player player) {
		...
		final Optional<BlockFace> optional = CompletableFuture.supplyAsync(() -> chooseDirection(yaw)).join();
		if (!optional.isPresent()) {
			Clan.ACTION.sendMessage(player, "Too diagonal! Please face a more specific direction and run this command again.");
	...
	public static void sendMapLoc(final Player player, Chunk chunk) {
		// TODO: Use the rotation to draw a compass rose
		// negative Z = NORTH
		final float yaw = player.getLocation().getYaw();
		final Optional<BlockFace> optional = CompletableFuture.supplyAsync(() -> chooseDirection(yaw)).join();

...
public class CommandRevoke extends ClanSubCommand {
	public CommandRevoke() {
		super("revoke");
		setUsage(ClansAPI.getDataInstance().getMessageString("Commands.revoke.text"));
	}
	...
	public boolean player(Player p, String label, String[] args) {
		...
		if (args.length == 0) {
			if (!Clan.ACTION.test(p, this.getPermission() + "." + DataManager.Security.getPermission("revoke")).deploy()) {
				lib.sendMessage(p, lib.noPermission(this.getPermission() + "." + DataManager.Security.getPermission("revoke")));
				return true;
			}
		...
		if (args.length == 2) {
			...
			if (associate != null) {
				...
				if (Clearance.MANAGE_PERMS.test(associate)) {
					...
					if (log.remove(target, r)) {
						lib.sendMessage(p, "&aClan permission &f" + target.getName() + " &awas removed from rank &6" + r.getName());
					} else {
						lib.sendMessage(p, "&cClan permission &f" + target.getName() + " &ccan't be removed from rank &6" + r.getName() + " &cbecause it's not directly inherited.");
					}

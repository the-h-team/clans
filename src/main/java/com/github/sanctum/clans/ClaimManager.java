		...
		ClanVentBus.subscribe(TimerEvent.class, Vent.Priority.HIGH, (e, subscription) -> {
			...
			if (Claim.ACTION.isAllowed().deploy()) {
				...
				if (!API.getClaimManager().isInClaim(p.getLocation())) {
					...
					if (res != null) {
						if (!res.getInfo().hasProperty(ResidentInformation.Trigger.LEAVING)) {
							res.getInfo().setProperty(ResidentInformation.Trigger.LEAVING, true);
					...
					if (!event.isCancelled()) {
						...
						r.getInfo().setProperty(ResidentInformation.Trigger.LEAVING, false); // ensure we're still in a claim, probably not needed but done anyways.
						...
						if (current.isActive()) {
							...
							if (!current.getId().equals(lastKnown.getId())) {
								if (r.getInfo().hasProperty(ResidentInformation.Trigger.NOTIFIED)) {
									if (!lastKnown.getOwner().getTag().getId().equals(r.getInfo().getCurrent().getOwner().getTag().getId())) {
										r.getInfo().setProperty(ResidentInformation.Trigger.TRAVERSED, true);
							...
							if (!r.getInfo().hasProperty(ResidentInformation.Trigger.NOTIFIED)) {
								...
								r.getInfo().setProperty(ResidentInformation.Trigger.NOTIFIED, true);
							} else {
								if (r.getInfo().hasProperty(ResidentInformation.Trigger.TRAVERSED)) {
									r.getInfo().setProperty(ResidentInformation.Trigger.TRAVERSED, false);
									r.getInfo().setTimeEntered(System.currentTimeMillis());
									OrdinalProcedure.of(event).run(0);
								}
							}
	...
	public boolean load(Claim claim) {
		...
		if (claim.getOwner() instanceof Clan) {
			...
			if (claim.getOwner() instanceof Clan.Associate) {
				if (claim.getOwner() instanceof PlayerAssociate) {
					PlayerAssociate associate = (PlayerAssociate) claim.getOwner();
